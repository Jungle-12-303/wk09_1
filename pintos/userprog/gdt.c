 ...
 * 사용할 수 있는 세그먼트들을 정의한다. 프로세스별 Local Descriptor
 * Table(LDT)도 존재하지만, 현대 운영체제에서는 사용하지 않는다.
 *
 * GDT의 각 엔트리는 테이블 안에서의 바이트 오프셋으로 식별되며,
 * 하나의 세그먼트를 나타낸다. 우리에게 중요한 세그먼트 타입은
 * 코드, 데이터, 그리고 TSS(Task-State Segment) 디스크립터 세 가지뿐이다.
 * 앞의 두 타입은 이름 그대로이며, TSS는 주로 인터럽트 시 스택 전환에 사용된다.
 */
