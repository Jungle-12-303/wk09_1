...
/* @lock 전역 디스크립터 테이블(GDT).
 *
 * GDT는 x86-64 전용 구조체로, 권한에 따라 시스템 안의 모든 프로세스가
 * 잠재적으로 사용할 수 있는 세그먼트를 정의한다. 프로세스별 Local
 * Descriptor Table(LDT)도 있지만, 현대 운영체제에서는 사용하지 않는다.
 *
 * GDT의 각 엔트리는 테이블 안의 바이트 오프셋으로 식별되며 세그먼트를
 * 나타낸다. 우리의 목적에서는 세 종류의 세그먼트만 중요하다. 코드, 데이터,
 * 그리고 TSS 또는 Task-State Segment 디스크립터다. 앞의 두 타입은 이름
 * 그대로이고, TSS는 주로 인터럽트 시 스택 전환에 사용된다. */
...
/* @lock 올바른 GDT를 설정한다. 부트스트랩 로더의 GDT에는 사용자 모드
   셀렉터나 TSS가 없었지만, 이제는 둘 다 필요하다. */
void
gdt_init (void) {
	/* @lock GDT를 초기화한다. */
