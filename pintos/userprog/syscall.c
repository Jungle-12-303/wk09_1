...
/* @bookmark [5] filesys_lock 전역 선언 (파일 시스템 동시 접근 방지)
 * 추가: struct lock filesys_lock 전역 변수
 ...
 * 메인 시스템 콜 인터페이스.
 */
/* @bookmark [7] syscall_handler: HALT/WRITE/EXIT 디스패치
 * 추가: switch(f->R.rax)로 SYS_HALT/WRITE/EXIT 분기 처리
 * 출처: 08db0db (args-none 테스트 통과) */
void
