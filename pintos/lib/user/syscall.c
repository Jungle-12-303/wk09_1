...
/* @lock 인자 없이 시스템 콜 NUMBER를 호출하고 반환값을 `int'로 반환한다. */
#define syscall0(NUMBER) ( \
		syscall(((uint64_t) NUMBER), 0, 0, 0, 0, 0, 0))

/* @lock 인자 ARG0를 넘겨 시스템 콜 NUMBER를 호출하고 반환값을 `int'로 반환한다. */
#define syscall1(NUMBER, ARG0) ( \
		syscall(((uint64_t) NUMBER), \
			((uint64_t) ARG0), 0, 0, 0, 0, 0))
/* @lock 인자 ARG0와 ARG1을 넘겨 시스템 콜 NUMBER를 호출하고 반환값을 `int'로 반환한다. */
