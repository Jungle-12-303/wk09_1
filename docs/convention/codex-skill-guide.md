# Codex Skill 등록 가이드

`docs/convention` 아래 문서들을 Codex가 직접 사용할 수 있는 스킬로 등록하고, 프롬프트에서 호출하는 방법을 정리한 가이드입니다.

## 대상 파일

이 저장소에서는 아래 파일을 스킬 원본으로 사용합니다.

- `docs/convention/ai-qa-convention.md`
- `docs/convention/c-coding-standard.md`
- `docs/convention/code-structure-convention.md`
- `docs/convention/comment-convention.md`
- `docs/convention/python-style.md`
- `docs/convention/project-structure.md`
- `docs/convention/commit-convention.md`
- `docs/convention/pr-convention.md`

이 파일들은 이미 YAML frontmatter의 `name`, `description`을 포함하고 있어 스킬 본문으로 재사용하기 좋습니다.

## Codex 스킬 기본 구조

Codex 스킬은 보통 `~/.codex/skills/<skill-name>/SKILL.md` 구조를 사용합니다.

예시:

```text
~/.codex/skills/
├── c-coding-standard/
│   └── SKILL.md
├── code-structure-convention/
│   └── SKILL.md
├── comment-convention/
│   └── SKILL.md
├── python-style/
│   └── SKILL.md
├── project-structure/
│   └── SKILL.md
├── commit-convention/
│   └── SKILL.md
└── pr-convention/
    └── SKILL.md
```

핵심은 파일명보다 폴더 안의 `SKILL.md`입니다.
즉 `docs/convention/c-coding-standard.md`를 그대로 두고,
그 내용을 `~/.codex/skills/c-coding-standard/SKILL.md`로 복사하면 됩니다.

## 등록 방법

### 1. 스킬 폴더 만들기

```bash
mkdir -p ~/.codex/skills/c-coding-standard
mkdir -p ~/.codex/skills/code-structure-convention
mkdir -p ~/.codex/skills/comment-convention
mkdir -p ~/.codex/skills/python-style
mkdir -p ~/.codex/skills/project-structure
mkdir -p ~/.codex/skills/commit-convention
mkdir -p ~/.codex/skills/pr-convention
```

### 2. 컨벤션 파일을 `SKILL.md`로 복사하기

현재 저장소 루트에서 실행:

```bash
cp docs/convention/c-coding-standard.md ~/.codex/skills/c-coding-standard/SKILL.md
cp docs/convention/code-structure-convention.md ~/.codex/skills/code-structure-convention/SKILL.md
cp docs/convention/comment-convention.md ~/.codex/skills/comment-convention/SKILL.md
cp docs/convention/python-style.md ~/.codex/skills/python-style/SKILL.md
cp docs/convention/project-structure.md ~/.codex/skills/project-structure/SKILL.md
cp docs/convention/commit-convention.md ~/.codex/skills/commit-convention/SKILL.md
cp docs/convention/pr-convention.md ~/.codex/skills/pr-convention/SKILL.md
```

### 3. 한 번에 등록하기

```bash
mkdir -p \
  ~/.codex/skills/c-coding-standard \
  ~/.codex/skills/code-structure-convention \
  ~/.codex/skills/comment-convention \
  ~/.codex/skills/python-style \
  ~/.codex/skills/project-structure \
  ~/.codex/skills/commit-convention \
  ~/.codex/skills/pr-convention

cp docs/convention/c-coding-standard.md ~/.codex/skills/c-coding-standard/SKILL.md
cp docs/convention/code-structure-convention.md ~/.codex/skills/code-structure-convention/SKILL.md
cp docs/convention/comment-convention.md ~/.codex/skills/comment-convention/SKILL.md
cp docs/convention/python-style.md ~/.codex/skills/python-style/SKILL.md
cp docs/convention/project-structure.md ~/.codex/skills/project-structure/SKILL.md
cp docs/convention/commit-convention.md ~/.codex/skills/commit-convention/SKILL.md
cp docs/convention/pr-convention.md ~/.codex/skills/pr-convention/SKILL.md
```

## 업데이트 방법

이 저장소의 `docs/convention/*.md`를 원본으로 유지하고,
문서를 수정한 뒤 다시 `~/.codex/skills/.../SKILL.md`로 복사하면 됩니다.

예시:

```bash
cp docs/convention/pr-convention.md ~/.codex/skills/pr-convention/SKILL.md
```

새 세션에서 반영하는 것이 가장 안전합니다.
이미 열려 있는 Codex 세션이 있다면 새로 열거나 재시작해서 스킬 목록을 다시 읽게 하면 됩니다.

## 사용하는 방법

스킬 이름을 프롬프트에 직접 언급하면 가장 확실합니다.

### 예시 프롬프트

#### commit-convention

```text
commit-convention 스킬로 이번 변경에 맞는 커밋 메시지를 작성해줘
```

```text
브랜치 전략은 commit-convention 기준으로 따라줘
```

#### pr-convention

```text
pr-convention 스킬로 이번 변경에 맞는 PR 제목과 설명을 작성해줘
```

```text
이 브랜치 dev에 올릴 PR 설명을 pr-convention 기준으로 정리해줘
```

#### c-coding-standard

```text
c-coding-standard 스킬 기준으로 Pintos C 코드를 정리해줘
```

```text
포인터 선언, 들여쓰기, 함수 작성 방식은 c-coding-standard 컨벤션을 따라줘
```


#### code-structure-convention

```text
code-structure-convention 기준으로 새 helper 함수 위치를 잡아줘
```

```text
전역 변수와 static 함수 배치는 code-structure-convention을 따라줘
```

#### comment-convention

```text
comment-convention 기준으로 Pintos 영어 주석을 한글로 번역해줘
```

```text
새 주석을 추가할 때 comment-convention의 @lock 규칙을 지켜줘
```

#### python-style

```text
python-style 스킬 기준으로 이 모듈을 정리해줘
```

```text
pytest 테스트 코드도 python-style에 맞춰 작성해줘
```

#### project-structure

```text
project-structure 스킬 기준으로 폴더 구조를 다시 잡아줘
```

```text
이 파일은 어디에 두는 게 맞는지 project-structure 기준으로 판단해줘
```

## 여러 스킬을 함께 쓰는 방법

필요하면 한 요청에서 여러 개를 같이 부를 수 있습니다.

예시:

```text
project-structure와 c-coding-standard 스킬을 같이 적용해서 C 프로젝트 골격을 만들고,
마지막 커밋 메시지는 commit-convention 기준으로 작성해줘
```

```text
commit-convention과 pr-convention을 함께 적용해서 커밋 메시지와 PR 설명을 작성해줘
```

```text
c-coding-standard, code-structure-convention, comment-convention을 함께 적용해서
Pintos thread.c를 정리해줘
```

## 권장 운영 방식

- 저장소 안의 `docs/convention/*.md`를 팀의 원본 문서로 유지합니다.
- 개인 환경의 `~/.codex/skills/*/SKILL.md`는 실행용 복사본으로 봅니다.
- 컨벤션 문서를 바꿨으면 스킬 복사본도 함께 갱신합니다.
- 스킬 호출이 중요할 때는 프롬프트에 이름을 직접 적습니다.

## 빠른 점검 체크리스트

- `~/.codex/skills/<skill-name>/SKILL.md`가 존재하는가
- 파일 상단 frontmatter에 `name`, `description`이 있는가
- 스킬 이름을 프롬프트에 직접 언급했는가
- 문서 수정 후 새 Codex 세션에서 다시 확인했는가
